...
This feature relies on support from the Flutter tools which means it will only show emulators when using a very recent Flutter SDK (only the master channel at the time of publishing). It may take a little while to filter through to the `dev` and `beta` channels.
...
This feature relies on support from the Dart analysis server which means it will only activate when using a very recent SDK. Flutter's `master` channel already has this change, but it may take a little while to filter through to the `dev` and `beta` channels.
